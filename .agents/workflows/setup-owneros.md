---
description: Set up OwnerOS on this Mac from chat alone. Clones or updates the repo, checks prerequisites, asks three short questions one at a time, runs the installer, confirms the cockpit is live in the browser, and starts the owner's first job. The owner never touches a terminal; you run every command.
---
...
terminal at any point. You run every command yourself, show them what matters, and
ask one question at a time in plain words. Read `AGENTS.md` in the OwnerOS folder
first and keep its rules. Do not redesign anything.
...
OwnerOS. Otherwise OwnerOS lives at `~/OwnerOS`. Safe to run more than once:
...
- claude missing: continue. Say Brock's briefing and the Files assistant stay quiet
  until Claude Code is installed and signed in; every room still loads. Offer to
  install it with the official installer (`curl -fsSL https://claude.ai/install.sh | bash`);
...
- If step 1 said brand-missing, the first job is the brand conversation, and it
  happens right here. Read `~/.claude/skills/crew-core-brand-context/SKILL.md` and
  follow it exactly, including its Step 0 and Final Step. It is a short
